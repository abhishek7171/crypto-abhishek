...
def get_from_ticker(msg):
    r = requests.get("https://api.coindcx.com/exchange/ticker")
    ticker_dict = {}
    resp = ''
    if r.status_code == 200:
        data = r.json()
        if 'all' in msg.lower():
            for each_tick in data:
                if "INR" in each_tick['market']:
                    if each_tick['market'] not in ticker_dict:
                        ticker_dict[each_tick['market']] = {k:v for k,v in each_tick.items() if k not in 'market'}
        elif 'eth' in msg.lower():
            for each_tick in data:
                if 'INR' in each_tick['market'] and 'ETH' in each_tick['market']:
                    if each_tick['market'] not in ticker_dict:
                        ticker_dict[each_tick['market']] = {k: v for k, v in each_tick.items() if k not in 'market'}
        else:
            resp = 'pls tell which coin to check for or type ticker all to get for all markets'
    else:
        resp = "Not able to fetch the details for tinker, api is down. pls try later"

    if ticker_dict:
        resp = str(ticker_dict)
...
def bot():
    ...
    if 'ticker' in incoming_msg.lower():
        text = get_from_ticker(incoming_msg)
        msg.body(text)
        responded = True
